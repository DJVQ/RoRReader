...
import static com.example.myreadproject8.util.file.FileUtils.BLANK;
...
public class LocalPageLoader extends PageLoader {
    ...
    public LocalPageLoader(PageView pageView, Book collBook, ChapterService mChapterService, Setting setting, List<BookMarkE> bookMarkE,String isRead) {
        super(pageView, collBook, setting,bookMarkE,isRead);
    ...
    private List<Chapter> loadChapters() throws IOException {
        ...
        while ((length = bookStream.read(buffer, 0, buffer.length)) > 0) {//一个byte一个byte的读取,read返回ascii码十进制值
            //移动块位置
            ++blockPos;//（一个byte为一个块)

            if (hasChapter) {//若匹配到标题
                //将数据转换成String
                String blockContent = new String(buffer, 0, length, mCharset);
                System.out.println("mtestsss"+blockContent);
                int lastN = blockContent.lastIndexOf("\n");//找到最后出现的\n
                System.out.println("mtestsss"+lastN);
                if (lastN != 0) {
                    blockContent = blockContent.substring(0, lastN);//裁剪
                    length = blockContent.getBytes(mCharset).length;
                    allLength = allLength + length;//计算总长
                    bookStream.seek(allLength);//重新定位
                }
    ...
    private boolean checkChapterType(RandomAccessFile bookStream) throws IOException {
        ...
        //进行章节匹配
        for (String str : CHAPTER_PATTERNS) {//匹配章节标题关键字
            Pattern pattern = Pattern.compile(str, Pattern.MULTILINE);//匹配多行
