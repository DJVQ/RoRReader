...
import com.example.myreadproject8.util.read.SnackbarUtils;
import com.example.myreadproject8.widget.animation.AutoPageAnim;
import com.example.myreadproject8.widget.animation.CoverPageAnim;
import com.example.myreadproject8.widget.animation.CoverVerticalPageAnim;
import com.example.myreadproject8.widget.animation.HorizonPageAnim;
import com.example.myreadproject8.widget.animation.NonePageAnim;
import com.example.myreadproject8.widget.animation.PageAnimation;
import com.example.myreadproject8.widget.animation.ScrollPageAnim;
import com.example.myreadproject8.widget.animation.SimulationPageAnim;
import com.example.myreadproject8.widget.animation.SlidePageAnim;
import com.example.myreadproject8.widget.animation.VerticalPageAnim;
...
public class PageView extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mPageLoader != null) {
            mPageLoader.prepareDisplay(w, h);//预置页面
        }
    }
    ...
    private void init() {
        ...
        mTextSelectPaint.setAntiAlias(true);//抗锯齿
    ...
    void setPageMode(PageMode pageMode) {
        ...
        switch (mPageMode) {
            ...
            case NONE:
                ...
                break;
            case SCROLL:
                mPageAnim = new ScrollPageAnim(mViewWidth, mViewHeight, 0, mPageLoader.getMarginTop(), mPageLoader.getMarginBottom(),this, mPageAnimListener);
                break;
            case AUTO:
    ...
    public void drawNextPage() {
        ...
        if (mPageAnim instanceof HorizonPageAnim) {//水平翻页模式
            ((HorizonPageAnim) mPageAnim).changePage();
        }else if (mPageAnim instanceof VerticalPageAnim){//垂直翻页模式
            ((VerticalPageAnim) mPageAnim).changePage();
        } else if (mPageAnim instanceof AutoPageAnim) {//自动翻页模式
     ...
     * @return
     */
    public PageLoader getPageLoader(Book collBook, ReadCrawler mReadCrawler, Setting setting, List<BookMarkE> bookMarkE,String isRead) {
        // 判是否已经存在
        if (mPageLoader != null) {
            return mPageLoader;
        }
        ...
        if ("本地书籍".equals(collBook.getType())) {
            mPageLoader = new LocalPageLoader(this, collBook, ChapterService.getInstance(), setting,bookMarkE,isRead);
        } else {
